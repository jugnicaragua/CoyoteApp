...
interface ComercialBanksInterface {
    ...
    suspend fun requestExchangeRateByDate(@Path("dateSearch") dateSearch: String): RootBank

    @GET("api/commercialBankExchangeRates/date?")
    suspend fun requestExchangeRateByDateRange(@QueryMap(encoded = true) dateMap: HashMap<String, Any>): RootBank
